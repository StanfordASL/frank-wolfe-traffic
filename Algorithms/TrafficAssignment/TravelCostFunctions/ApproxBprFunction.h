...
class ApproxBprFunction {
 public:
  ...
  double operator()(const int e, const double x) const {
	  ...
	  {
		  x_new = x + exo*cap;
		  const double pt =XPT * cap;
		  if (x_new <= pt)
			  return bpr(e, 0);
		  else
		  {
			  b_slope = (bpr(e, 2*cap) - bpr(e, pt)) / (2*cap - pt);
			  return b_slope * (x_new - pt) +  bpr(e, 0);
		  }
	  }
	  else
	  {
  ...
  double integral(const int e, const double b) const {
	  ...
	  std::cout << "INTEGRAL OF APPROXBPR IS USED" << std::endl;	  
	  return 0.0;
  }
  ...
  Vec4d operator()(const int e, const Vec4d& x) const {
	  ...
	  Vec4d val_real = (bpr(e, 0.0*slope) + slope * (max(x_new, pt_fake) - pt_fake)) * edgeReal;
 ...
 private:
	...
	CustomBprFunction<GraphT> cbpr; // Custom BPR function.
	double exo;
	int dummy_id;
	Vec4d exo_v;
};
